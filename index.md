---
okf_version: "0.2"
---

# Base de conocimiento del CAU del CEPRUD

Este repositorio organiza el conocimiento utilizado para atender y resolver consultas e incidencias del CAU del CEPRUD.

## Servicios

### PRADO

- [Índice de PRADO](prado/index.md)
- [Espacios docentes](prado/espacios-docentes/index.md)
- [Usuarios y roles](prado/usuarios-y-roles/index.md)
- [Conceptos y reglas](prado/conceptos-y-reglas/index.md)
- [Procedimientos y casuísticas](prado/procedimientos/index.md)
- [Categorías de IRIS](prado/iris/index.md)
- [Arquitectura de la información y procesos](prado/arquitectura-y-procesos/index.md)
- [Temas de ayuda de IRIS](prado/temas-de-ayuda/index.md)

### ABIERTA UGR

- [CAU de ABIERTA UGR](abierta/index.md)

### E-CAMPUS

- [CAU de E-CAMPUS](ecampus/index.md)

### OCW

- [CAU de OCW](ocw/index.md)

### Formación

- [CAU de Formación](formacion/index.md)

### Otros servicios

- [CAU de Otros servicios](otros/index.md)

## Gestión del repositorio

- [Pendientes de documentación](pendientes-documentacion.md)
- [Guía de revisión y mantenimiento](guia-revision-mantenimiento.md)
- [Historial de cambios](log.md)
